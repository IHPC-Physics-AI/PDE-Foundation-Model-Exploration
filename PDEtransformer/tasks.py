...
def generate_channel_mask(channel: torch.Tensor, prob: float, patch_size: int) -> list[torch.Tensor]: # noqa
    #channel of shape tensor(1,2,256,256)
    batch_size = channel.shape[0] #=1
    #patch_size=4
    num_channel_tokens = (channel.shape[2] // patch_size) * (channel.shape[3] // patch_size) # 4096
    num_channel_tokens = int(num_channel_tokens)

    mask = torch.ones(batch_size, num_channel_tokens,
                      device=channel.device) #shape (1,4096)

    mask = torch.bernoulli(prob * mask) #prob=0 for empty, prob=1 for full
...
class ForwardPrediction(AbstractTask):
    ...
    def __init__(self, patch_size: int, num_timesteps: int, m: int, n: int, task_idx: int):
        """
        m to n forward prediction task #1 to 1
        Args:
            :param patch_size:
        """
        super(ForwardPrediction, self).__init__()
        self.patch_size = patch_size
        self.m = m #1 input frame
        self.n = n #1 pred frame
        self.num_timesteps = num_timesteps #=m+n
        self.task_idx = task_idx
    ...
    def prepare_data(self, data: dict, prob: float) -> dict:
        ...
        for channel in channels:
            channel_id = channel['channel_id']
            channel = channel['channel'] #(1,2,256,256)
            num_timesteps = channel.shape[1] #=2

            time_inactive = generate_empty_channel_mask(channels=channel, patch_size=self.patch_size) #tensor(1,4096) of all zeros
            time_active = generate_full_channel_mask(channels=channel, patch_size=self.patch_size) #tensor(1,4096) of all ones

            time_masks = [time_active] * (self.num_timesteps - self.n) #list of 1, tensor(1,4096) all ones
            time_masks += [time_inactive] * self.n #Now, list of 2, tensor (1,4096) of all ones, and tensor(1,4096) of all zeros

            task_emb = (torch.zeros(channel.shape[0], device=channel.device)
                        + self.task_idx) #tensor([0]), shape tensor(1,)
            task_emb = task_emb.long()

            simulation_time = torch.einsum('t, b -> b t',
                                           torch.arange(0, num_timesteps, device=channel.device),
                                           data['time_step_stride']) #tensor shape(1,2), tensor([[0, 1]])
...
class TaskMasking(nn.Module):
    ...
    def get_channels(self, batch: dict, idx: int) -> Tuple[torch.Tensor, torch.Tensor]: # noqa
        ...
        Args:
            batch: batch data, at this point, still has physical metadata and loading metadata
        ...
        channel_data: torch.Tensor = batch['data'][:, :, idx] #channel_data = tensor(1,30,256,256), for channels(idx) 0 and 1 respectively

        if self.downsampling_factor > 1: #self.downsampling_factor = 1
            channel_data = avg_pool2d(channel_data, self.downsampling_factor)

        if len(batch['physical_metadata']['Fields'].shape) == 1: #len = 2
            channel_id = batch['physical_metadata']['Fields'].long()
        else:
            channel_id = batch['physical_metadata']['Fields'][:, idx].long() #tensor([8]) for idx 0, tensor([9]) for idx 1
    ...
    def get_data(self, batch: dict):
        ...
        num_channels = batch['data'].shape[2] #num_channels = 2
        ...
        pde = batch['physical_metadata']['PDE'] #tensor([[10]])
        constants = batch['constants']  #shape tensor(1,5), tensor([[0.0400, 0.0600, 0.0000, 0.0000, 0.0000]]), feed rate/kill rate
        constants_class = batch['physical_metadata']['Constants'] #tensor(1,5), tensor([[18, 19,  0,  0,  0]])
        time_step_stride = batch['time_step_stride'].flatten() #tensor([[1]])

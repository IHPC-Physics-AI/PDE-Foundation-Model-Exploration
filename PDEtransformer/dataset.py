...
config = {
    ...
    "local_datasets_dir": "/Volumes/T7/New_Data/2d_gs_rd/data",
    "global_dataset_dir": "/Volumes/T7/New_Data/2d_gs_rd/data",
    "dataset_ext": ".hdf5"
...
class Dataset:
    def __init__(
        ...
        **kwargs,
        ...
        if all_time_steps: #False
            ...
            self.intermediate_time_steps = intermediate_time_steps or False # True
            self.trim_start = trim_start or 0 # = 0

            self.trim_end = trim_end or 0 # = 0
            self.step_size = step_size or 1 # = 1
    ...
    def __getitem__(self, idx):
        ...
        const = get_sel_const_sim(self.dset, sim_idx, self.sel_const) #feed rate/kill rate

        input_frame_idx = (
                self.trim_start + (idx % self.samples_per_sim) * self.step_size  #Train:input_frame_idx = 0 + (idx mod 29) * 1, Test:input_frame_idx = 0 + (idx mod 1) * 1 = 0 always
        )
        target_frame_idx = input_frame_idx + self.time_steps  # target_frame_idx = input_frame_idx + 1

        dim_list = self.sim_shape[2:]

        if self.crop_size is None: # crop_size = None
        ...
        if self.norm_strat_data:
            input = self.norm_strat_data.normalize(input) # input = (2,256,256)

            if self.intermediate_time_steps:
                target = np.array(
                    [self.norm_strat_data.normalize(frame) for frame in target]
                ) # target = (1,2,256,256) for train, loops through for test
            else:
        ...
        ) #This is what is returned for line 145 of metadataset_dataset, from_loader

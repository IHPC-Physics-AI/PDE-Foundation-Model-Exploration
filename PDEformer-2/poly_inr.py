...
class PolyINR(nn.Cell):
    ...
    def __init__(self,
        ...
        # activation function
        activation_fn = activation_fn.lower() # sin
    ...
    def construct(self,
        ...
        x_pad = self.coord_pad(x)  # [bsz, n_pts, dim_in] -> [bsz, n_pts, dim_in+1], for DCR/MVDCR: (1,65536,4) -> (1,65536,5)

        for layer_idx in range(self.num_layers - 1):
            if scale_modulations is None: # (11,1,256)
                scale = 1.0  # (1,1,256)
            else:
                scale = 1. + scale_modulations[layer_idx].expand_dims(1)  # [bsz, 1, dim_hidden], i.e. (1,1,256)

            if shift_modulations is None: # (11,1,256)
                shift = 0.0  # (1,1,256)
            else:
                shift = shift_modulations[layer_idx].expand_dims(1)  # [bsz, 1, dim_hidden], i.e. (1,1,256)

            tmp = self.affine_act(self.affines[layer_idx](x))  # [bsz, n_pts, dim_hidden], i.e. (1,65536,256)
            if affine_modulations is not None: #affine_modulations=None
            ...
            hidden_state = hidden_state * tmp  # [bsz, n_pts, dim_hidden], hidden_state = 1.0, shape: (1,65536,256)
            hidden_state = self.dense_layers[layer_idx](hidden_state)  # [bsz, n_pts, dim_hidden], shape: (1,65536,256)
            hidden_state = scale * hidden_state + shift   #shape: (1,65536,256)
            hidden_state = self.act(hidden_state)  # [bsz, n_pts, dim_hidden], shape: (1,65536,256)

        #Add pseudo_inverse here?
        out = self.last_layer(hidden_state)  # [bsz, n_pts, dim_out], shape: (1, 65536, 1)
...
class PolyINRWithHypernet(nn.Cell):
    ...
    def __init__(
        ...
        self.inr = PolyINR(
            dim_in=inr_dim_in, # 4
            dim_hidden=inr_dim_hidden, # 256
            dim_out=inr_dim_out, # 1
            num_layers=inr_num_layers, # 12
            modify_he_init=modify_he_init, # False
            activation_fn=activation_fn, # sin
            affine_act_fn=affine_act_fn, # identity
    ...
    def construct(self, coordinate: Tensor, hyper_in: Tensor) -> Tensor:
        ...
        #affine_modulations: None
        #scale_modulations: (11,1,256)
        #shift_modulations: (11,1,256)
        #coordinate: (1,65536, 4)
        out = self.inr(coordinate, affine_modulations, scale_modulations, shift_modulations)  # (1, 65536, 1)
        return out

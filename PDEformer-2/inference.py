...
def inference_pde(model: PDEformer,
    ...
    """
    coordinate = txyz_coord.astype(np.float32).reshape((-1, 4)) # (65536,4), same shape for both DCR/MVDCR
    ...
    pred = model(as_tensor(pde_dag.node_type), as_tensor(pde_dag.node_scalar),
                 ...
                 as_tensor(pde_dag.spatial_pos), as_tensor(coordinate)) # Goes from inference.py to pdeformer.py/PDEformer.construct
    pred = pred.asnumpy().astype(np.float32)  # [1, n_pts, 1],  DCR/MVDCR: (1,65536, 1)
    ...
    if pde_dag.n_vars > 1:
        ...
        for idx_var in range(1, pde_dag.n_vars):
            spatial_pos, attn_bias = pde_dag.get_spatial_pos_attn_bias(idx_var) # Both of shape (216,216), this serves as the identifier for variables (See pde_dag.py/PDEAsDAG for more info)
            pred = model(
    ...
    pred = pred.reshape(txyz_coord.shape[:-1] + (pde_dag.n_vars,))  #DCR/MVDCR: (4,128,128,n_vars)
    return pred
...
def infer_plot_2d(model: PDEformer,
    ...
    # [n_t, dim1, dim2, n_vars], i.e. (n_t, 128,128,4)

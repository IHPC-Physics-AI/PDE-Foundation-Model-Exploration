...
class PDEformer(nn.Cell):
    ...
    def construct(self,
        ...
        #Same shapes for both DCR/MVDCR
        #pde_feature: (216,1,768)
        #node_type: (1,216,1)
        #node_scalar: (1,192,1)
        #node_function: (1,6,16384,5)
        #in_degree: (1,216)
        #out_degree: (1,216)
        #attn_bias: (1,216,216)
        #spatial_pos: (1,216,216)

        # Shape is [n_graph, num_points, dim_out].
        #self.n_inr_nodes = 11
        out = self.inr(coordinate, pde_feature[:self.n_inr_nodes]) # DCR/MVDCR: (1, 65536, 1)
        if self.multi_inr: # False

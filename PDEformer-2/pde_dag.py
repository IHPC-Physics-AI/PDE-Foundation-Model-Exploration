...
class PDEAsDAG:
    ...
    def __init__(self,
        ...
        num_spatial = config.model.graphormer.num_spatial #=16
        disconn_attn_bias = float(config.data.pde_dag.disconn_attn_bias) #=-inf
        # Number of INR modulation nodes for each uf node.
        if config.model.multi_inr.enable and config.model.multi_inr.separate_latent: #multi_inr.enable: False,
            uf_num_mod = config.model.inr.num_layers + config.model.inr2.num_layers - 2
        else:
            uf_num_mod = config.model.inr.num_layers - 1  #=11
        function_num_branches = config.model.function_encoder.num_branches #=4
        max_n_scalar_nodes = config.data.pde_dag.max_n_scalar_nodes #=192
        max_n_function_nodes = config.data.pde_dag.max_n_function_nodes #=6
    ...
    def plot(self, label: str = 'type', hide: str = 'pad') -> None:
        ...
        nx.draw_networkx_edges(self._dag, pos, edge_list, edge_color='white')
        nx.draw_networkx_labels(self._dag, pos, labels_dict, font_color='black')
        plt.axis('off')
        plt.show()
...
class PDENodesCollector:
    ...
    def new_uf(self, domain: Optional[PDENode] = None) -> PDENode:
        ...
        if domain is None: #True, domain is None
            return self._add_node('uf') #creates and adds uf node to node_list, with value of scalar 0 to node_scalar
    ...
    def set_ic(self,
        ...
        self._add_function(field_values, t=0., x=x, y=y, z=z) #Property nodes under PDENode are added here
    ...
    def _add_node(self,
        ...
        if name == "": #If no name, is assigned, then this step here gives the node a name
    ...
    def _add_function(self,
        ...
        #t is set to 0 in set_ic
        #For DCR:, t,x,y,z are all broadcasted to (128,128), f_values is already (128,128) -> after stacking: (128,128,5)
        # [*, 5] -> [n_pts, 5]
        function = function.reshape((-1, 5)) #(128,128,5) -> (128^2,5) = (16384,5)

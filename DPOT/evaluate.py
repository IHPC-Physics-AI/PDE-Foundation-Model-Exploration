...
with torch.no_grad():
    for id, test_loader in enumerate(test_loaders):
        ...
        for batch_idx, (xx, yy, msk, _) in enumerate(test_loader):
            ...
            #
            # # Use only the first sample in the batch for the animation
            # sample_idx = 0
            #
            # # Setup figure for the two channels
            # fig, axes = plt.subplots(2, 2, figsize=(12, 10))
            #
            # # Titles and initial frames
            # # Row 0: Channel 0 (Species U) | Row 1: Channel 1 (Species V)
            # ims = []
            # for c in range(2):
            #     axes[c, 0].set_title(f'Ground Truth (Ch {c})')
            #     axes[c, 1].set_title(f'Model Prediction (Ch {c})')
            #
            #     im_gt = axes[c, 0].imshow(gt_np[sample_idx, ..., 0, c], cmap='viridis' if c==0 else 'magma', animated=True)
            #     im_pred = axes[c, 1].imshow(pred_np[sample_idx, ..., 0, c], cmap='viridis' if c==0 else 'magma', animated=True)
            ...
            #     updated_plots = []
            #     for c in range(2):
            #         ims[c][0].set_array(gt_np[sample_idx, ..., t_idx, c])
            #         ims[c][1].set_array(pred_np[sample_idx, ..., t_idx, c])
            #         updated_plots.extend([ims[c][0], ims[c][1]])
            #     return updated_plots
            #
            # # Create animation: 3001 steps / 20 = ~150 frames
            # ani = FuncAnimation(fig, update, frames=range(pred_np.shape[-2] // 1), blit=True)
            #
            # gif_path = os.path.join(desktop_plots_dir, f'sim{batch_idx:03d}_animation.gif')
            ...
            # timesteps_to_plot = list(range(0, num_timesteps, 100))  # [0,10,20,...,90]
            # if (num_timesteps - 1) not in timesteps_to_plot:
            #     timesteps_to_plot.append(num_timesteps - 1)
            #
            # for c in range(num_channels):
            #     fig, axs = plt.subplots(len(timesteps_to_plot), 3, figsize=(18, 5 * len(timesteps_to_plot)))
            #
            #     for row, time_idx in enumerate(timesteps_to_plot):
            #         pred_plot = pred_np[sample_idx, ..., time_idx, c]
            #         gt_plot = gt_np[sample_idx, ..., time_idx, c]
            #         error_plot = np.abs(pred_plot - gt_plot)
            #
            #         im0 = axs[row, 0].imshow(gt_plot, cmap='viridis')
            #         axs[row, 0].set_title(f'GT | T: {time_idx} | Ch: {c}')
            #         fig.colorbar(im0, ax=axs[row, 0], fraction=0.046, pad=0.04)
            #
            #         im1 = axs[row, 1].imshow(pred_plot, cmap='viridis')
            #         axs[row, 1].set_title(f'Pred | T: {time_idx} | Ch: {c}')
            #         fig.colorbar(im1, ax=axs[row, 1], fraction=0.046, pad=0.04)
            #
            #         im2 = axs[row, 2].imshow(error_plot, cmap='magma')
            #         axs[row, 2].set_title(f'Abs Error | T: {time_idx} | Ch: {c}')
            #         fig.colorbar(im2, ax=axs[row, 2], fraction=0.046, pad=0.04)
            #
            #     # inside for c, outside for row
            #     plt.suptitle(f'Simulation {batch_idx} | Channel {c}', fontsize=16, y=1.001)
            #     plt.tight_layout()
            #     save_path = os.path.join(desktop_plots_dir, f'sim{batch_idx:03d}_ch{c}_all_timesteps.png')

...
from prose.prose_fd.data_utils.cfdbench import get_auto_dataset


def preprocess_cfdbench_data():
    train_data_cavity, dev_data_cavity, test_data_cavity = get_auto_dataset(
        data_dir=Path("/Volumes/T7/CFDBench"),
    ...
    with h5py.File("/Volumes/T7/CFDBench/PROSE-FD_Processed_check/ns2d_cdb_train.hdf5", "w") as fp:
        fp.create_dataset("data", data=train_data, compression=None)

    with h5py.File("/Volumes/T7/CFDBench/PROSE-FD_Processed_check/ns2d_cdb_test.hdf5", "w") as fp:
        fp.create_dataset("data", data=test_data, compression=None)

    with h5py.File("/Volumes/T7/CFDBench/PROSE-FD_Processed_check/ns2d_cdb_val.hdf5", "w") as fp:

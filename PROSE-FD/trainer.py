...
class Trainer(object):
    def __init__(self, modules, params, symbol_env):
        ...
        if not params.use_raw_time:
            ...
            if params.rollout:
                ...
                self.t = torch.linspace(0, 10, params.data.t_num, dtype=torch.float32)[None]  # (1, t_num) # this is where self.t is, 10/(20-1), since data.t_num=20 by default
    ...
    def data_loss_fn(self, data_output, data_label, data_mask):
        ...
        elif self.params.loss_weight == "linf":
...
#BREAKPOINT THIS TIME AND MODEL INPUT
    #TRACE SYMBOLIC ENCODER LATER
    def prepare_data(self, samples, train=True):
        ...
        if self.params.use_raw_time: # use_raw_time = 0
            t = samples["t"]
        else:
            t = self.t

        input_len = self.params.input_len  # 10 by default in main.yaml, first 10 frames as input
        data_input = data[:, :input_len]  # (bs, input_len, x_num, x_num, dim)

        # prepare inputs for operator / 1 step training

        data_label = data[:, input_len:]  # (bs, output_len, x_num, x_num, dim), # output_len = 10, last 10 frames as ground truth
